...
def equity_curve_with_long_and_short(df, leverage_rate=3, c_rate=5.0/1000, min_margin_rate=0.15):
    """
    ...
    # ===计算仓位变动
    # 开仓时仓位，这里控制每次开仓仓位一样，48，70，74，75，81总共5行修改净值计算方式，改为固定金额开仓，按1%计算
    # df.loc[open_pos_condition, 'position'] = init_cash * leverage_rate * (1 + df['buy_at_open_change'])  # 建仓后的仓位
    df.loc[open_pos_condition, 'position'] = 1000 * (1 + df['buy_at_open_change'])    # 固定资金，每次10元钱
    ...
    # ===计算每天实际持有资金的变化
    # 计算持仓利润，固定比例开仓时，这里也需要调整为固定值
    # df['profit'] = (df['position'] - init_cash * leverage_rate) * df['pos']  # 持仓盈利或者损失
    df['profit'] = (df['position'] - 1000) * df['pos']

    # 计算持仓利润最小值，固定比例开仓时，调整为固定值
    ...
    df.loc[df['pos'] == 1, 'profit_min'] = (df['position_min'] - 1000) * df['pos']  # 最小持仓盈利或者损失
    df.loc[df['pos'] == -1, 'profit_min'] = (df['position_max'] - 1000) * df['pos']  # 最小持仓盈利或者损失

    # 计算实际资金量
    df['cash'] = init_cash + df['profit']  # 实际资金
    # df['cash'] -= init_cash * leverage_rate * c_rate  # 减去建仓时的手续费
    df['cash'] -= 1000 * c_rate  # 减去建仓时的手续费，固定比例开仓时，调整为固定值
